...
//			test.setContentNo(i);
//			test.setMember(userRepository.findByUserNo(i-40));
//			contentLikeLogRepository.save(test);
//		}

		for (int i = 0; i < 5; i++) {
			ColoLog test = new ColoLog();
			test.setVote(RlTypeEnum.RIGHT);
			test.setMember(userRepository.findByUserNo(i+15));
			test.setBoardColo(coloBoardRepository.findByColoNo(i));
			test.setCancelYn("N");
			coloLogRepository.save(test);
		}
